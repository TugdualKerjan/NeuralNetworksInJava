...
public class neuronPanel extends JPanel {
	...
	protected void paintComponent(Graphics g) {
		...
		//Cut up the image into n + 1 sections, n being the layers plus the input and output image
		double layerWidth = (double) width / (double) (network.length + 3);
		...
		for(int y = 0; y < 28; y++) {
			for(int x = 0; x < 28; x++) {
				g.setColor(new Color((int) (255 * neuronValues[network.length - 1][y * 28 + x]), (int) (255 * neuronValues[network.length - 1][y * 28 + x]), (int) (255 * neuronValues[network.length - 1][y * 28 + x])));
				g.fillRect(xCenter + x * pixelImageSize + (int) (layerWidth * (network.length + 1)), yCenter + y * pixelImageSize, pixelImageSize, pixelImageSize);
			}
		}
		...
		for(int layer = 0; layer < network.length; layer++) {
			...
			for(int neuron = 0; neuron < network[layer].length; neuron++) {
				...
				for(int weight = 0; weight < network[layer][neuron].length; weight++) {
					g.setColor(new Color((int) (128 + 110 * (network[layer][neuron][weight] / biggestWeight)), 0, (int) (128 - 110 * (network[layer][neuron][weight]) / biggestWeight)));
